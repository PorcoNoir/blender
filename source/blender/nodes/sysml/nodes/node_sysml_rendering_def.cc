...
static void sysml_rendering_def_init(bNodeTree *ntree, bNode *node)
{
  ...
  node->color[0] = 0.4627f;
  node->color[1] = 0.7216f;
  node->color[2] = 0.851f;
  node->flag |= NODE_CUSTOM_COLOR;  /* family accent */
}

void register_node_type_sysml_rendering_def()
{
  static bke::bNodeType ntype;

  sysml_node_type_base(&ntype, "SysMLNodeRenderingDef"_ustr);
  ntype.ui_name = "Rendering Definition";
  ntype.ui_description = "SysML v2 rendering definition";
  ntype.nclass = NODE_CLASS_INPUT;
  ntype.initfunc = sysml_rendering_def_init;
