...
static void sysml_comment_init(bNodeTree *ntree, bNode *node)
{
  sysml_node_storage_init(node);
  bke::node_add_socket(*ntree, *node, SOCK_OUT, "NodeSocketSysMLElement", "self", "Self");
}

void register_node_type_sysml_comment()
{
  static bke::bNodeType ntype;

  sysml_node_type_base(&ntype, "SysMLNodeComment"_ustr);
  ntype.ui_name = "Comment";
  ntype.ui_description = "SysML v2 comment";
  ntype.nclass = NODE_CLASS_INPUT;
  ntype.initfunc = sysml_comment_init;
