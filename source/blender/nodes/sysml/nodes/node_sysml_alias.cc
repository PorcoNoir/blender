...
static void sysml_alias_init(bNodeTree *ntree, bNode *node)
{
  sysml_node_storage_init(node);
  bke::node_add_socket(*ntree, *node, SOCK_OUT, "NodeSocketSysMLElement", "self", "Self");
}

void register_node_type_sysml_alias()
{
  static bke::bNodeType ntype;

  sysml_node_type_base(&ntype, "SysMLNodeAlias"_ustr);
  ntype.ui_name = "Alias";
  ntype.ui_description = "SysML v2 alias";
  ntype.nclass = NODE_CLASS_INPUT;
  ntype.initfunc = sysml_alias_init;
