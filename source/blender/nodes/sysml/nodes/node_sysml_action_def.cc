...
static void sysml_action_def_init(bNodeTree *ntree, bNode *node)
{
  ...
  node->color[0] = 0.6549f;
  node->color[1] = 0.4275f;
  node->color[2] = 0.7098f;
  node->flag |= NODE_CUSTOM_COLOR;  /* family accent */
}

void register_node_type_sysml_action_def()
{
  static bke::bNodeType ntype;

  sysml_node_type_base(&ntype, "SysMLNodeActionDef"_ustr);
  ntype.ui_name = "Action Definition";
  ntype.ui_description = "SysML v2 action definition";
  ntype.nclass = NODE_CLASS_INPUT;
  ntype.initfunc = sysml_action_def_init;
