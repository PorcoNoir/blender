...
static void sysml_flow_usage_init(bNodeTree *ntree, bNode *node)
{
  ...
  bke::node_add_socket(*ntree, *node, SOCK_IN, "NodeSocketSysMLElement", "from", "From");
  bke::node_add_socket(*ntree, *node, SOCK_IN, "NodeSocketSysMLElement", "to", "To");
}

void register_node_type_sysml_flow_usage()
{
  static bke::bNodeType ntype;

  sysml_node_type_base(&ntype, "SysMLNodeFlowUsage"_ustr);
  ntype.ui_name = "Flow Usage";
  ntype.ui_description = "SysML v2 flow usage";
  ntype.nclass = NODE_CLASS_INPUT;
  ntype.initfunc = sysml_flow_usage_init;
